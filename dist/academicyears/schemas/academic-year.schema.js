"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AcademicYearSchema = exports.AcademicYear = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
let AcademicYear = class AcademicYear {
    user;
    name;
    startDate;
    endDate;
    schools;
    isCurrent;
    status;
    description;
    isArchived;
};
exports.AcademicYear = AcademicYear;
__decorate([
    (0, mongoose_1.Prop)({ type: mongoose_2.Types.ObjectId, ref: 'User', required: true, index: true }),
    __metadata("design:type", mongoose_2.Types.ObjectId)
], AcademicYear.prototype, "user", void 0);
__decorate([
    (0, mongoose_1.Prop)(),
    __metadata("design:type", String)
], AcademicYear.prototype, "name", void 0);
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", Date)
], AcademicYear.prototype, "startDate", void 0);
__decorate([
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", Date)
], AcademicYear.prototype, "endDate", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: [{ type: mongoose_2.Types.ObjectId, ref: 'School' }] }),
    __metadata("design:type", Array)
], AcademicYear.prototype, "schools", void 0);
__decorate([
    (0, mongoose_1.Prop)({ default: false, index: true }),
    __metadata("design:type", Boolean)
], AcademicYear.prototype, "isCurrent", void 0);
__decorate([
    (0, mongoose_1.Prop)({ enum: ['planned', 'active', 'completed', 'cancelled'], default: 'planned' }),
    __metadata("design:type", String)
], AcademicYear.prototype, "status", void 0);
__decorate([
    (0, mongoose_1.Prop)({ maxlength: 500 }),
    __metadata("design:type", String)
], AcademicYear.prototype, "description", void 0);
__decorate([
    (0, mongoose_1.Prop)({ default: false, index: true }),
    __metadata("design:type", Boolean)
], AcademicYear.prototype, "isArchived", void 0);
exports.AcademicYear = AcademicYear = __decorate([
    (0, mongoose_1.Schema)({ timestamps: true })
], AcademicYear);
exports.AcademicYearSchema = mongoose_1.SchemaFactory.createForClass(AcademicYear);
//# sourceMappingURL=academic-year.schema.js.map