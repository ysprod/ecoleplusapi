...
export class CantineService {
  ...
  async getMenusBySchoolId(schoolId: string) {
    ...
    if (!Types.ObjectId.isValid(schoolId)) throw new NotFoundException('Invalid schoolId');
    const mesmenus = await this.repository.getMenusBySchool(new Types.ObjectId(schoolId));
    return mesmenus;
  }
  ...
  async updateMenu(data: CantineUpdateDto) {
    if (!Types.ObjectId.isValid(data.id)) throw new NotFoundException('Invalid id');
    const updated = await this.repository.updateMenu(new Types.ObjectId(data.id), data);
